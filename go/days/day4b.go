...
[1518-06-06 23:56] Guard #3271 begins shift`
	...
	// at this point, entries are sorted

	sleep_counts := make(map[int]map[int]int)
	...
	for idx, entry := range entries {
		...
		if entry.action == wake_up {
			...
			for i := asleep_at; i < awake_at ; i++ {
				_, ok := sleep_counts[most_recent_id]
				if ok {
					sleep_counts[most_recent_id][i]++
				} else {
					sleep_counts[most_recent_id] = make(map[int]int)
				}
			}
	...
	id_of_most_asleep := -1
	most_asleep := 0
	for key, value := range sleep_counts {
		for key2, value2 := range value {
			if value2 > most_asleep {
				most_asleep = value2
				id_of_most_asleep = key
				min_at_which_most_asleep = key2
			}
		}
	}
	fmt.Println(min_at_which_most_asleep, id_of_most_asleep, most_asleep)
	fmt.Println(min_at_which_most_asleep*id_of_most_asleep)
